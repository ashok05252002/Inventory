...
export const Customers = () => {
  ...
  const columns: Column<CustomerModel>[] = [
    { 
      header: 'Code', 
      accessor: (row) => (
        <Link to={`/customers/${row.code}`} className="font-bold text-indigo-600 hover:text-indigo-800 hover:underline transition-colors">
          {row.code}
        </Link>
      ) 
    },
    { header: 'Customer Name', accessor: 'name', className: 'font-semibold text-slate-800' },
    { 
      header: 'Contact', 
      accessor: (row) => (
  ...
  return (
    <div className="space-y-6 h-full flex flex-col animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Customer Master</h1>
          <p className="text-sm text-slate-500 mt-1 font-medium">Manage your customer database and credit terms.</p>
        </div>
      ...
      <Modal 
        ...
        title={modalMode === 'create' ? 'Create New Customer' : 'Edit Customer'}
        footer={modalFooter}
      >
